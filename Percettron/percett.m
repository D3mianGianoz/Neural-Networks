...
X(:,m+1)=-ones(t,1); %array di -1 nella colonna m+1
storiapesi=zeros(m+1,n); %array di zeri
storiappresi=zeros(1,n);

for i=1:n
    storiapesi(:,i)=W'; 
    Y=segno(W*X'); %Y è un vettore riga che contiene le t uscite
    storiappresi(i)=sum(T'==Y); % in posizione i mette il totale degli esempi identificati correttamente
    for j=1:t 
        if ne(Y(j),T(j))
            W=W+T(j)*X(j,:); %regola di update
        end
    end
end
